# Universal auto-generated wrapper v3 for 'margin_premium'
from importlib import import_module
import pkgutil, inspect
import os, sys
_AGENT = "margin_premium"
VENDOR_PATH = os.path.join(os.path.dirname(__file__), "_vendor", _AGENT)
if VENDOR_PATH not in sys.path:
    sys.path.insert(0, VENDOR_PATH)
ROOT_PKG = "agents._vendor.margin_premium"
...
def run(payload=None):
    ...
    for _, fqname, entry, _ in _CANDS[:10]:  # ลองตัวเต็ง 10 อันดับแรก
        ...
        if isinstance(res, dict) and res.get("status")=="ERROR":
            ...
            continue
        # สำเร็จ
        return {"status":"OK","agent":"margin_premium","entry":fqname,"result":res}
    # ไม่เจออะไรเรียกได้
    try:
        ...
        exports = []
    return {"status":"NOOP","agent":"margin_premium","reason":"No callable entrypoint discovered","exports":exports}
